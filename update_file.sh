...
GITPOD_SUPERTOKEN_BACKEND_PORT=3000  # 這裡設置為你希望使用的端口，例如 3000
...
CONNECTION_URI="http://localhost:7007"
...
ENV_FILE_PATH="/workspace/gitpod-supertoken-demo/codeWorkspace/supertoken-supabase-demo/supertoken-supabase-demo-app/.env.development"
...
# 強制更新 NEXT_PUBLIC_GITPOD_SUPERTOKEN_BACKEND_URL
if grep -q "NEXT_PUBLIC_GITPOD_SUPERTOKEN_BACKEND_URL" "$ENV_FILE_PATH"; then
  # 如果變量已存在，使用 sed 替換
  sed -i "s|^NEXT_PUBLIC_GITPOD_SUPERTOKEN_BACKEND_URL=.*|NEXT_PUBLIC_GITPOD_SUPERTOKEN_BACKEND_URL=\"$GITPOD_SUPERTOKEN_BACKEND_URL\"|" "$ENV_FILE_PATH"
  echo "已更新 NEXT_PUBLIC_GITPOD_SUPERTOKEN_BACKEND_URL"
else
  # 如果變量不存在，則追加
  echo "NEXT_PUBLIC_GITPOD_SUPERTOKEN_BACKEND_URL=\"$GITPOD_SUPERTOKEN_BACKEND_URL\"" >> "$ENV_FILE_PATH"
  echo "已添加 NEXT_PUBLIC_GITPOD_SUPERTOKEN_BACKEND_URL"
fi

# 強制更新 NEXT_PUBLIC_GITPOD_SUPERTOKEN_WEB_URL
if grep -q "NEXT_PUBLIC_GITPOD_SUPERTOKEN_WEB_URL" "$ENV_FILE_PATH"; then
  # 如果變量已存在，使用 sed 替換
  sed -i "s|^NEXT_PUBLIC_GITPOD_SUPERTOKEN_WEB_URL=.*|NEXT_PUBLIC_GITPOD_SUPERTOKEN_WEB_URL=\"$GITPOD_SUPERTOKEN_WEB_URL\"|" "$ENV_FILE_PATH"
  echo "已更新 NEXT_PUBLIC_GITPOD_SUPERTOKEN_WEB_URL"
else
  # 如果變量不存在，則追加
  echo "NEXT_PUBLIC_GITPOD_SUPERTOKEN_WEB_URL=\"$GITPOD_SUPERTOKEN_WEB_URL\"" >> "$ENV_FILE_PATH"
  echo "已添加 NEXT_PUBLIC_GITPOD_SUPERTOKEN_WEB_URL"
fi

# 強制更新 CONNECTION_URI
if grep -q "CONNECTION_URI" "$ENV_FILE_PATH"; then
  # 如果變量已存在，使用 sed 替換
  sed -i "s|^CONNECTION_URI=.*|CONNECTION_URI=\"$CONNECTION_URI\"|" "$ENV_FILE_PATH"
  echo "已更新 CONNECTION_URI"
else
  # 如果變量不存在，則追加
  echo "CONNECTION_URI=\"$CONNECTION_URI\"" >> "$ENV_FILE_PATH"
...
\`${GITPOD_SUPERTOKEN_BACKEND_URL}/api/auth/dashboard\`
